...
class Model(torch.nn.Module):
    ...
    def _resolve_mxq(self, args: dict, mode: str) -> None:
        ...
        missing, `target` from `args` is required to compose the HF URL
        `mobilint/YOLO<rest>/<target>/<name>.mxq`. Hub artifacts are compiled in `all` core
        mode, so the path is not core-mode specific; `core_mode` is still required at inference
        and enforced by `MobilintBackend`.
        ...
        if not target:
            raise ValueError(
                f"Running '{mode}' on a missing '.mxq' requires 'target=' "
                f"(e.g. 'aries' | 'regulus') to locate the Hugging Face artifact. "
                f"Got target={target!r}."
            )
        resolved = attempt_download_mxq(self.model, device=target)
