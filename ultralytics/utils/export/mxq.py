...
# Mobilint HF repos follow the layout `mobilint/<RepoName>/<device>/<filename>.mxq`.
# Artifacts are compiled in `all` core mode, so a single `.mxq` serves every runtime
# core mode and the download path is not core-mode specific.
...
def attempt_download_mxq(file: str | Path, device: str) -> str:
    """Resolve a `.mxq` model path, downloading from Hugging Face Hub if needed.

    Hub artifacts are compiled in `all` core mode, so a single `.mxq` serves every
    runtime core mode and the download path is not core-mode specific.
    ...
    Resolution order:
      ...
      3. Derive the HF repo from the filename (`yolo*.mxq` -> `mobilint/YOLO*`) and
         download `<device>/<name>` into `<weights_dir>`.
    ...
    in_repo_path = f"{device}/{path.name}"
